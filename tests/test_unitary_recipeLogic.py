...
class RecipeTestCase(TestCase):
    ...
    def test_get_recipes_main(self):
        ...
        recipe1 = Recipe.objects.create(title='Recipe 1', ingredients="['Ingredient 1', 'Ingredient 2']", instructions="Instructions 1", creation_date ="2023-05-09", rating_average= '1.0' )
        recipe2 = Recipe.objects.create(title='Recipe 2', ingredients="['Ingredient 3', 'Ingredient 4']", instructions="Instructions 2", creation_date ="2022-04-01", rating_average= '5.0')
        ...
        self.assertEqual(len(recipes_rating), 2)  # Verifica que se han recuperado dos recetas

        recipes_recent = response['recipes_recent']
        self.assertEqual(len(recipes_recent), 2)  # Verifica que se han recuperado dos recetas

        # Verifica que los datos de las recetas coinciden con los que creaste

        #Ordenadas por rating
        self.assertEqual(recipes_rating[0]['title'], recipe2.title)
        self.assertEqual(recipes_rating[0]['ingredients'], recipe2.ingredients)
        self.assertEqual(recipes_rating[0]['instructions'], recipe2.instructions)
        self.assertEqual(recipes_rating[0]['creation_date'], recipe2.creation_date)
        self.assertEqual(recipes_rating[0]['rating_average'], recipe2.rating_average)

        self.assertEqual(recipes_rating[1]['title'], recipe1.title)
        self.assertEqual(recipes_rating[1]['ingredients'], recipe1.ingredients)
        self.assertEqual(recipes_rating[1]['instructions'], recipe1.instructions)
        self.assertEqual(recipes_rating[1]['creation_date'], recipe1.creation_date)
        self.assertEqual(recipes_rating[1]['rating_average'], recipe1.rating_average)

        #Ordenadas por recent
        self.assertEqual(recipes_recent[1]['title'], recipe1.title)
        self.assertEqual(recipes_recent[1]['ingredients'], recipe1.ingredients)
        self.assertEqual(recipes_recent[1]['instructions'], recipe1.instructions)
        self.assertEqual(recipes_recent[1]['creation_date'], recipe1.creation_date)
        self.assertEqual(recipes_recent[1]['rating_average'], recipe1.rating_average)

        self.assertEqual(recipes_recent[0]['title'], recipe2.title)
        self.assertEqual(recipes_recent[0]['ingredients'], recipe2.ingredients)
        self.assertEqual(recipes_recent[0]['instructions'], recipe2.instructions)
        self.assertEqual(recipes_recent[0]['creation_date'], recipe2.creation_date)
        self.assertEqual(recipes_recent[0]['rating_average'], recipe2.rating_average)

    def test_get_rating_by_id(self):
        #Crea dos recetas de ejemplo en la base de datos
        recipe1 = Recipe.objects.create(title='Recipe 1', id=1, ingredients="['Ingredient 1', 'Ingredient 2']", instructions="Instructions 1", creation_date ="2023-05-09", rating_average= 1.0, rating_amount=2 )
        recipe2 = Recipe.objects.create(title='Recipe 2', id=2, ingredients="['Ingredient 3', 'Ingredient 4']", instructions="Instructions 2", creation_date ="2022-04-01", rating_average= 5.0, rating_amount=3)
    ...
    def test_get_recipe_by_id(self):
        #Crea dos recetas de ejemplo en la base de datos
        recipe1 = Recipe.objects.create(title='Recipe 1', id=1, ingredients="['Ingredient 1', 'Ingredient 2']", instructions="Instructions 1")
